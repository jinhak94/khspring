...
<div id="menu-container">
     ...
     * 응답헤더에 Access-Control-Allow-Origin에
     * 현재 Origin을 허용할때는 CORS가 가능하다.
...
<div class="menu-test">
       ...
       <script>
        	$("#typeSelector").change(({target:{value}}) => {
        		$.ajax({
        			url: `\${menuRestApiHost}/menus/\${value}`,
        			method: "GET",
        			dataType: "json",
        			success: data => {
        				console.log(data);
...
$("#menuEnrollFrm").submit(e => {
	...
	//서버에 json 형식으로 데이터 전달(요청메세지 body)
	$.ajax({
...
<div class="menu-test">
 ...
 <script>
    $("#menuDeleteFrm").submit(e => {
   	e.preventDefault();
   	const id = Number($("[name=id]", e.target).val());
   	if(!id) return;
   	$.ajax({
   		url:  `\${menuRestApiHost}/menu/\${id}`,
   		method: "DELETE",
